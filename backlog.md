...
004. ~~cluade code lately have been calling the same tool over and over again in a loop. need to add a kill switch that would detect this and stop execution. or signal this to model~~ — done. sliding turn-based window (`ToolLoopMaxRepetitions` / `ToolLoopWindow`), `ErrToolLoop` sentinel, defaults 5/20.
